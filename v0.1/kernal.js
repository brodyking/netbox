...
function popupCreate(title, body) { // start function
	// generates post:
	document.body.innerHTML += '<div id="' + title + '" style="top: 30px; left: 5px;" class="popup" onmouseover="dragElement(document.getElementById(' + "'" + title + "'" + '));focused = ' + "'" + title + "'" + '"><div class="top" id="' + title + 'top">' + title + '<button style="float: right;margin-left: 3px;" onclick="popupClose(' + "'" + title + "'" + ')">X</button></div>' + body + '</div>';
}
...
commands["help"] = "termblock('Commands:<br>echo,clear,help,js')"; // rando command
...
function commandLookup() { // looks at the list of commands
	document.getElementById("terminaloutput").innerHTML += "<highlight>root@shitbox  $ " + document.getElementById("terminalinput").textContent + "</highlight><br>";
