...
var appsCurrentRepository = null;
...
commands["version"] = "terminline('<b>user: </b>root@netbox<br><b>window-manager: </b>" + wm + "<br><b>version: </b> 0.4<br><b>last-updated: </b> 03/12/2022<br><b>installed-apps: </b> " + appsCurrentlyInstalled + "')" 
...
function appsRepository() {
  appsCurrentRepository = document.getElementById('terminalinput').textContent.slice(3,Infinity);
  terminline("You set your current appsRepository to " + appsCurrentRepository)
}

function appsInstall() {
  var script = document.createElement('script'); // starts a script tag in head in html
  script.src = appsCurrentRepository + "/" + document.getElementById('terminalinput').textContent.slice(3,Infinity); // sets the SRC
  document.head.appendChild(script); // creates the script
  terminline("You installed " + document.getElementById('terminalinput').textContent.slice(3,Infinity) + " from " + appsCurrentRepository);
  appsCurrentlyInstalled += 1;
}

commands["ar"] = "appsRepository()";
commands["ai"] = "appsInstall()";